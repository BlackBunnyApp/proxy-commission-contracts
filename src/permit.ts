...
export async function getPermit(
	...
	if (compact) {
		...
		return (
			...
			(deadline === constants.MAX_UINT256.toString()
				? '00000000'
				: (BigInt(deadline) + 1n).toString(16).padStart(8, '0')) +
...
export async function getPermit2(
	...
	if (compact) {
		return (
			...
			expiration.toString(16).padStart(12, '0') +
			BigInt(nonce).toString(16).padStart(12, '0') +
			sigDeadline.toString(16).padStart(64, '0') +
			BigInt(r).toString(16).padStart(64, '0') +
			(BigInt(s) | (BigInt(v - 27) << 255n)).toString(16).padStart(64, '0')
		);
	}
...
export async function getPermitLikeDai(
	...
	if (compact) {
		...
		return (
			...
			(expiry === constants.MAX_UINT256.toString()
				? '00000000'
				: (BigInt(expiry) + 1n).toString(16).padStart(8, '0')) +
			BigInt(r).toString(16).padStart(64, '0') +
			(BigInt(s) | (BigInt(v - 27) << 255n)).toString(16).padStart(64, '0')
		);
	}
